...
def _get_pipeline():
    ...
    if _pipeline is not None:  # fast path — no lock needed
        return _pipeline
    with _lock:
        if _pipeline is not None:  # re-check under lock
            return _pipeline
        if _load_failed:
            return None
        try:
...
def classify_batch(texts: list[str]) -> list[dict]:
    ...
    # Guard against length mismatch from partial batch exceptions
    if len(results) != len(texts):
        logger.error("classify_batch result count mismatch: %d vs %d", len(results), len(texts))
        results.extend([{"label": "neutral", "score": 0.0} for _ in range(len(texts) - len(results))])
    return results
